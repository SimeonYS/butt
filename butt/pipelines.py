...
class BbuttPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('butt.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `butt`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM butt WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO butt (date, title, link, content)"
